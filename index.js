...
debug:function(e){try{fs.appendFileSync(`./log/log.html`,`<br><pre><b style="color:#0022ff;background-color:#000000;font-family:monospace;">`+"["+month+"-"+date+"-"+year+"*"+hours+"."+minutes+"."+seconds+"]"+`[debug]>>>`+e+`</pre></b>\n`);}catch(e){};console.log('\x1b[36m'+e+'\x1b[0m');},
info:function(e){try{fs.appendFileSync(`./log/log.html`,`<br><pre><b style="color:#ffffff;background-color:#000000;font-family:monospace;">`+"["+month+"-"+date+"-"+year+"*"+hours+"."+minutes+"."+seconds+"]"+`[info]>>>`+e+`</pre></b>\n`);}catch(e){};console.log('\x1b[97m'+e+'\x1b[0m');},
WARN:function(e){try{fs.appendFileSync(`./log/log.html`,`<br><pre><b style="color:#ff9900;background-color:#000000;font-family:monospace;">`+"["+month+"-"+date+"-"+year+"*"+hours+"."+minutes+"."+seconds+"]"+`[WARN]>>>`+e+`</pre></b>\n`);}catch(e){};console.log('\x1b[33m'+e+'\x1b[0m');},
      ...
      if (message[0]===config.chat.commandprefix){
        ...
        switch (message.split(' ')[0]){
          ...
          /*case'/cred':if(socket.proto.admin||socket.proto.moderator||socket.proto.owner){try{
            var currentJSON=require(`./public/accounts/${message.split(' ')[1]}.json`);
          }catch(e){socket.emit('message',{name:'server',message:e});logger.ERROR(e);}
            
            socket.emit('message',{name:'server',message:currentJSON})
          }else{
...
if(config.server.manAuthStartup===false){SERV();}else{logger.trace('!!!waiting for passcode!!!');readline.question('enter startup passcode:\n> ', function (name) {config.server.authVerify=name;readline.close();});readline.on('close', function () {try{if((config.server.manAuthStartup!==false)&&(sha256(config.server.authVerify)===config.server.manAuthStartup)){SERV();logger.trace('!!!passcode accepted!!!');}}catch(e){logger.FATAL(e)}});}
