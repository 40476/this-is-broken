...
var today = new Date().getHours();if((document.location.host==='bakchatfork.glitch.me')&&(([0,6].indexOf(new Date().getDay()) != -1)||(!(today >= 7 && today <= 14)))){document.location="https://bakchat.40476.repl.co"}else{console.log(((document.location.host==='bakchatfork.glitch.me')&&(([0,6].indexOf(new Date().getDay()) != -1)||(today >= 7 && today <= 19))))}
...
function check(){fetch('bans/' + username + '.txt').then(response => response.text()).then(data => { proc(data); }); }
function initIcons(){fetch('/../images/fav-unread-pink.txt').then(response => response.text()).then(data => { pinkIcon=data }); }
...
setInterval(() => { if ((!focus) && (unread !== 0)) { if (__osc === 1) { $('#icon').prop('href', 'images/fav.png'); __osc = 0; } else { $('#icon').prop('href','images/fav-pink.png'); __osc = 1; } } }, 280);
